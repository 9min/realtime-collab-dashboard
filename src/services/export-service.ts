...
const CSV_HEADERS = [
  '제목', '설명', '우선순위', '담당자', '상태(컬럼)',
  '마감일', '서브태스크', '라벨', '생성일', '수정일',
] as const
...
function rowToCsv(row: ExportRow): string {
  return [
    row.title, row.description, row.priority, row.assignee,
    row.column, row.dueDate, row.subtasks, row.labels,
...
async function fetchExportData(
  ...
  const [tasksResult, columnsResult, membersResult, subtasksResult, labelsResult, taskLabelsResult] =
    await Promise.all([
      ...
      supabase.from('task_labels').select('task_id, label_id, labels!inner(project_id)').eq('labels.project_id', projectId),
    ])

  const tasks = (tasksResult.data ?? []) as Tables<'tasks'>[]
  const columns = (columnsResult.data ?? []) as Tables<'kanban_columns'>[]
  const members = (membersResult.data ?? []) as Array<{ user_id: string; profiles: { full_name: string | null; email: string } }>
  ...
  const memberMap = new Map(members.map((m) => [m.user_id, m.profiles.full_name ?? m.profiles.email]))
