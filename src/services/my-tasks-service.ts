...
export async function getMyTasks(
  ...
  let query = supabase
    .from('tasks')
    .select('*, projects:project_id(name), kanban_columns:column_id(title, is_done_column)')
  ...
  const tasks: MyTaskWithProject[] = (data ?? []).map((t) => {
    ...
    const columns = raw.kanban_columns as { title: string; is_done_column: boolean } | null
    return {
      id: raw.id,
      ...
      is_done_column: columns?.is_done_column ?? false,
